...
struct AeFilter {
    ...
    void setCutoff(float f, float q, int type) {
        float w0 = 2*M_PI*f/APP->engine->getSampleRate();
...
struct AeFilterFrame : AeFilter {
    dsp::Frame<CHANNELS> x[2];
    dsp::Frame<CHANNELS> y[2];
    ...
    dsp::Frame<CHANNELS> process(dsp::Frame<CHANNELS> in) {
    	dsp::Frame<CHANNELS> out;
    	for(int i=0;i<channels;i++) {
